...
main(int argc, char** argv)
  ...
  // Load EEG data inside eeg.bin
  // We loose the dimensionality here so these needs to be passed in another file
  arma::mat eeg_data;
  eeg_data.load("eeg.data", arma::raw_binary);
  eeg_data.print();

  // Need to pass around some info (in a text file also would work)
  arma:mat eeg_info;
  eeg_info.load("eeg.info", arma::raw_binary);
  eeg_info.print();

  // TODO: Reshape the matrix

  cout << "Number Surrogate: " << argv[1] << endl;
  cout << "P Value: " << argv[2] << endl;
